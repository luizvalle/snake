...
namespace snake_game {
    SDLGraphics::SDLGraphics() : m_window{nullptr}, m_renderer{nullptr} {
         if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER)) {
             std::string error_message = std::string("Error initializing SDL: ")
    ...
    void SDLGraphics::create_window(const char *title,
                                    unsigned int x,
                                    unsigned int y,
                                    unsigned int w,
                                    unsigned int h) {
        m_window = SDL_CreateWindow(title, x, y, w, h, 0);
        if (!m_window) {
            std::string error_message = std::string("Error creating window: ")
                                        + std::string(SDL_GetError());
            throw std::runtime_error(error_message);
        }
        ...
        if (!m_renderer) {
            std::string error_message = std::string("Error creating renderer: ")
                                        + std::string(SDL_GetError());
            throw std::runtime_error(error_message);
        }
    }
